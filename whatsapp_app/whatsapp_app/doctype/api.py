...
def check_generate_pdf():
   ...
   # create a dictionary to store equipment information for each supplier
    ...
    for supplier, equipment_list in supplier_dict.items():
        ...
        if whatsapp_no is not None and name_of_supplier is not None:
            file_path = whatsapp_no + "_" + name_of_supplier.replace(" ", "_").lower() + '.pdf'
        pdf = weasyprint.HTML(string=message).write_pdf()
        ...
        url = f"{api_endpoint}/{name_type}/{version}/sendTemplateMessage?whatsappNumber=919979060444"
        payload = {
            "parameters": [
                {
                    "name": "supp_name",
    ...
    # send reminder if compliance expired in 7 days.
    print("\n\n supplier reminder", reminder_supllier)
    supplier_unique_list = list(set(reminder_supllier))
    print("\n\n supplier unique list", supplier_unique_list)
    for r_supplier in supplier_unique_list:
        whatsapp_no = frappe.db.get_value("Supplier", filters={'name': r_supplier}, fieldname=["whatsapp_no"])  
        if whatsapp_no is None:
            continue
        print("\n\n asdfasdf", whatsapp_no)
        url = f"{api_endpoint}/{name_type}/{version}/sendTemplateMessage?whatsappNumber=919979060444"
        # payload = {
        #     "broadcast_name": "compliance_update",
        #     "template_name": "compliance_update"

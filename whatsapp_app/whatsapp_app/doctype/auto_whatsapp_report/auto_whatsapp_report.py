...
from whatsapp_app.whatsapp_app.doctype.api import whatsapp_keys_details


class AutoWhatsappReport(Document):
    def autoname(self):
        self.name = _(self.report)
        if frappe.db.exists("Auto Whatsapp Report", self.name):
            self.name = append_number_if_name_exists("Auto Whatsapp Report", self.name)
    ...
    def validate_report_count(self):
        count = frappe.db.count("Auto Whatsapp Report", {"user": self.user, "enabled": 1})
    ...
    def get_report_content(self):
        ...
        if self.format == "HTML":
            ...
            xlsx_file = make_xlsx(xlsx_data, "Auto Whatsapp Report", column_widths=column_widths)
    ...
    def get_html_table(self, columns=None, data=None):
        ...
        return date_time, report_doctype
        ...
        #         "edit_report_settings": get_link_to_form("Auto Whatsapp Report", self.name),
        #     },
        # )
    ...
    def send(self):
        ...
        # print("\n\n report attachment type", type(attachments), "\n\n")
        # print("\n\n report message", message, "\n\n")
        # print("\n\n report message type", type(message), "\n\n")

        from frappe.email.doctype.email_queue.email_queue import QueueBuilder

        attachments = self.get_file_name()
        ...
        mobile_no = self.whatsapp_to
        mobile_nos = " ".join(mobile_no.split()).split()
        print("\n\n mobile no", mobile_nos, "\n\n")
        for mobile in mobile_nos:
            mobile = int(mobile)
            print("\n\n attachment", attachments, "\n\n")
            #
            # attachments = QueueBuilder.get_attachments(self)
            # print("\n\n attachments", attachments, "\n\n")
            ...
            url = f"{api_endpoint}/{name_type}/{version}/sendSessionMessage/91{mobile}?messageText=filename={attachments}"
            print("\n\n url", url)
...
def download(name):
    ...
    auto_whatsapp_report = frappe.get_doc("Auto Whatsapp Report", name)
    auto_whatsapp_report.check_permission()
    ...
    frappe.local.response.filename = auto_whatsapp_report.get_file_name()


@frappe.whitelist()
def send_now(name):
    """Send Auto Whatsapp Report now"""
    auto_whatsapp_report = frappe.get_doc("Auto Whatsapp Report", name)
    auto_whatsapp_report.check_permission()
    auto_whatsapp_report.send()
...
def send_daily():
    ...
    enabled_reports = frappe.get_all(
        "Auto Whatsapp Report", filters={"enabled": 1, "frequency": ("in", ("Daily", "Weekdays", "Weekly"))}
    )

    for report in enabled_reports:
        auto_whatsapp_report = frappe.get_doc("Auto Whatsapp Report", report.name)

        # if not correct weekday, skip
        if auto_whatsapp_report.frequency == "Weekdays":
            if current_day in ("Saturday", "Sunday"):
                continue
        elif auto_whatsapp_report.frequency == "Weekly":
            if auto_whatsapp_report.day_of_week != current_day:
                continue
        try:
            auto_whatsapp_report.send()
        except Exception as e:
            auto_whatsapp_report.log_error(f"Failed to send {auto_whatsapp_report.name} Auto Whatsapp Report")


def send_monthly():
    """Check reports to be sent monthly"""
    for report in frappe.get_all("Auto Whatsapp Report", {"enabled": 1, "frequency": "Monthly"}):
        frappe.get_doc("Auto Whatsapp Report", report.name).send()
